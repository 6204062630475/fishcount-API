...
async function loadModel() {
  ...
  // warming up model
  const dummyInput = tf.zeros(yolov5.inputs[0].shape);
...
app.post("/upload", (req, res) => {
  ...
  if (base64String) {
    ...
    const inputImage = tfNode.node.decodeImage(imageBuffer); // สร้างรูปภาพจาก Uint8Array
    ...
    detectImage(inputImage, model, classThreshold);
    res.send("detected");
  } else {
